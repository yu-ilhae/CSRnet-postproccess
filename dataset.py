...
class PairedResizeCrop(object):
    '''
    Resize and Crop for both image and its density map.
    If the image/density map is larger than 512x512, crop the center to 512x512.
    If it is smaller, resize it to 512x512.
    '''
    ...
    def resize_crop(self, img):
        if img.size[0] >= self.target_size[0] and img.size[1] >= self.target_size[1]:
            ...
            # Resize to 512x512 if the image is smaller
...
def create_train_dataloader(root, use_flip, batch_size):
    ...
    '''
    main_trans_list = [PairedResizeCrop(target_size=(512, 512))]
    if use_flip:
